...
import com.example.appquanlidiem.bean_tkb.Time;
import com.example.appquanlidiem.help_tkb.OneClickListener;

import com.example.appquanlidiem.ui_tkb.coursedetails.CourseDetailsActivity;
import com.example.appquanlidiem.ui_tkb.editcourse.EditActivity;

import com.example.appquanlidiem.ui_tkb.settime.SetTimeActivity;
import com.example.appquanlidiem.util_tkb.CalendarReminderUtils;
import com.example.appquanlidiem.util_tkb.Config;
import com.example.appquanlidiem.util_tkb.DialogUtils;
import com.example.appquanlidiem.util_tkb.ExcelUtils;
import com.example.appquanlidiem.util_tkb.FileUtils;
import com.example.appquanlidiem.util_tkb.Utils;
import java.io.File;
...
public class ThoiKhoaBieuMainActivity extends AppCompatActivity {
    ...
    private void setCalendarEvent() {
        ...
        if (CalendarReminderUtils.checkPermission(this)) {
            ...
            if (CalendarReminderUtils.checkAndAddCalendarAccount(this) == -1) {
                Toast.makeText(this, "Không có tài khoản trong lịch, tạo tự động không thành công, vui lòng tạo thủ công", Toast.LENGTH_LONG).show();
            } else {
    ...
    private void addClassCalendarEvent(List<Course> courses, final long start) {
        ...
        String [] weeks = new String [] {"CN", "Thứ Hai", "Thứ Ba", "Thứ Tư", "Thứ Năm", "Thứ Sáu", "Thứ Bảy"};
        for (Course course : courses) {
            ...
            if (!classStart.isEmpty() && !classEnd.isEmpty()) {
                ...
                if (uri == null) {
                    Toast.makeText(this,
                            weeks[course.getDayOfWeek() - 1] + "-" + course.getName() + "Không thêm được lịch biểu", Toast.LENGTH_SHORT).show();
                } else {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
      ...
      if (id == R.id.menu_set_week) {
            showSelectCurrentWeekDialog();
        } else if (id == R.id.menu_append) {
            ...
            switch (requestCode) {
                case REQUEST_CODE_FILE_CHOOSE:
                    ...
                    if (!FileUtils.getFileExtension(name).equals("xls")) {
                        DialogUtils.showTipDialog(this, "Vui lòng chọn một tệp Excel có phần mở rộng là xls");
                        return;
                    }
